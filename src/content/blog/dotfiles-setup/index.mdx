...
A few things aren't automated. You do these once after bootstrapping:

- **Oh My Zsh** — install if not present
- **`zsh-syntax-highlighting` plugin** — clone into OMZ custom plugins
...
| --- | --- | --- |
| `ls` | `eza --color --icons --long --header --git --time-style=relative --group-directories-first` | Long listing with git status |
